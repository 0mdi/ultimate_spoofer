...
bool get_process_alpc_port_handle(uint32_t pid, HANDLE& alpc_handle)
{
    ...
    for (i = 0; i < handleInfo->HandleCount; i++)
    {
        ...
        if (objectName.Length)
        {
            ...
            if (std::wstring(objectTypeInfo->Name.Buffer).find(L"ALPC Port") != std::wstring::npos &&
                std::wstring(objectName.Buffer).find(L"RPC Control") != std::wstring::npos)
            {
...
bool deny_wmic()
{
    ...
    system("wmic diskdrive get serialnumber,model");

    // read pid from registry
    DWORD alpc_pid = 0;
    if (!RegReadDword(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Wbem\\Transports\\Decoupled\\Server", L"ProcessIdentifier", alpc_pid))
    {
        return false;
    }
